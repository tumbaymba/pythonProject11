...
def create_table(params_db):
    ...
    with conn.cursor() as cur:
        try:
            cur.execute("""
                CREATE TABLE companies (company_id int primary key, company_name varchar unique not null)
                        """)
    ...
    with conn.cursor() as cur:
        try:
            cur.execute("""
                CREATE TABLE vacancies (vacancy_id serial primary key, vacancy_name text not null,
                                        salary int,
